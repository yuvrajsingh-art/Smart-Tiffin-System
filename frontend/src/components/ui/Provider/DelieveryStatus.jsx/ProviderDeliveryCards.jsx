                            ...
                            <div className="lg:w-64 space-y-3">
                                ...
                                <div className="flex flex-col gap-2 mt-2">
                                    {delivery.status === 'preparing' && (
                                        <button
                                            onClick={() => onUpdateStatus(delivery.id, 'mark_ready')}
                                            className="w-full bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 transition-colors text-sm font-semibold"
                                        >
                                            Mark Ready
                                        </button>
                                    )}
                                    {delivery.status === 'ready_for_pickup' && (
                                        <button
                                            onClick={() => onUpdateStatus(delivery.id, 'mark_dispatched')}
                                            className="w-full bg-orange-500 text-white py-2 px-4 rounded-lg hover:bg-orange-600 transition-colors text-sm font-semibold"
                                        >
                                            Mark Dispatched
                                        </button>
                                    )}
                                    {delivery.status === 'out_for_delivery' && (
                                        <button
                                            onClick={() => onUpdateStatus(delivery.id, 'mark_delivered')}
