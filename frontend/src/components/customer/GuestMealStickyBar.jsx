...
const GuestMealStickyBar = ({ totalGuests, totalCost, onCheckout }) => {
    ...
    return (
        <div className="fixed bottom-4 left-4 right-4 z-[9000] max-w-7xl mx-auto">
            <div className="bg-[#2D241E] text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        className="px-6 py-2.5 rounded-xl font-bold text-sm border border-white/20 hover:bg-white/10"
                    >
                        Book (Pay Later)
                    </button>
                    <button
                        onClick={() => onCheckout(true)}
                        className="bg-white text-[#2D241E] px-6 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2"
                    >
                        Pay Now ₹{totalCost}
