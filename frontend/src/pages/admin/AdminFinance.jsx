        ...
        <div className="space-y-6 max-w-[1600px] mx-auto min-h-full pb-8 animate-[fadeIn_0.5s]">
            ...
            <div className="w-full bg-[#2D241E] text-white overflow-hidden py-1.5 rounded-xl shadow-lg flex items-center gap-4 px-4 relative z-0">
                <div className="flex items-center gap-1 shrink-0 z-10 bg-[#2D241E] pr-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-emerald-400">Finance Pulse</span>
                </div>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 className="text-xl font-black text-[#2D241E] tracking-tight mb-1">Finance & Billing</h2>
                    <p className="text-[#5C4D42] text-xs font-medium opacity-80 flex items-center gap-2">
                        <span className="size-2 rounded-full bg-emerald-500 animate-pulse"></span>
                        Books Open • FY 2024-25
                    </p>
                </div>

                {/* Top Navigation Tabs */}
                <div className="bg-white/40 p-1 rounded-xl border border-white/60 flex gap-1 backdrop-blur-sm self-start md:self-auto">
                    {['Overview', 'Payouts', 'Invoices', 'Tax'].map(t => (
                        <button
                            key={t}
                            onClick={() => setActiveTab(t)}
                            className={`px-5 py-2 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${activeTab === t ? 'bg-[#2D241E] text-white shadow-lg scale-105' : 'text-[#897a70] hover:bg-white'}`}
                        >
                            {t}
                        </button>
                    ))}
    ...
    <div className="space-y-6 animate-slide-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-[#2D241E] to-[#453831] p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                ...
                <p className="text-[9px] font-black text-white/40 uppercase tracking-[0.2em] mb-4">Total Liquidity</p>
                <h3 className="text-5xl font-black tracking-tighter">₹8.42L</h3>
            ...
            <div className="glass-card p-8 rounded-[2.5rem] flex flex-col justify-between group hover:border-orange-200/50 transition-colors">
                <div className="flex justify-between items-start">
                    <p className="text-[9px] font-black text-[#5C4D42]/60 uppercase tracking-[0.2em]">Pending Payouts</p>
                ...
                <div>
                    <h3 className="text-3xl font-black text-[#2D241E] mt-2">₹3.15L</h3>
                    <p className="text-[9px] font-bold text-[#897a70] mt-1">Due for 12 Kitchens</p>
                </div>
            ...
            <div className="glass-card p-8 rounded-[2.5rem] flex flex-col justify-between group hover:border-blue-200/50 transition-colors">
                <div className="flex justify-between items-start">
                    <p className="text-[9px] font-black text-[#5C4D42]/60 uppercase tracking-[0.2em]">Net Profit (Jan)</p>
                ...
                <div>
                    <h3 className="text-3xl font-black text-[#2D241E] mt-2">₹1.12L</h3>
                    <p className="text-[9px] font-bold text-[#897a70] mt-1">After tax & commissions</p>
                </div>
                <div className="w-full h-1.5 bg-gray-100 rounded-full mt-4 overflow-hidden">
        ...
        <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white/60 p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-base font-black text-[#2D241E]">Recent Movements</h3>
                <button onClick={handleViewLedger} className="text-[9px] font-bold text-[#897a70] hover:text-[#2D241E] transition-colors">View All Ledger</button>
            </div>
                ...
                {payoutsData.slice(0, 3).map((p, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-100 hover:scale-[1.01] transition-transform cursor-pointer" onClick={() => openSettlement(p)}>
                        <div className="flex items-center gap-4">
                            <div className="size-10 bg-gray-50 rounded-xl flex items-center justify-center text-[#2D241E] font-black text-xs">{p.kitchen.charAt(0)}</div>
                            <div>
                                <p className="text-xs font-bold text-[#2D241E]">{p.kitchen}</p>
                                <p className="text-[9px] font-medium text-[#897a70] uppercase tracking-wide">Payout • {p.date}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-xs font-black text-[#2D241E]">{p.amount}</p>
                            <p className={`text-[9px] font-bold ${p.status === 'Paid' ? 'text-emerald-500' : 'text-amber-500'}`}>{p.status}</p>
                        </div>
                    </div>
                ))}
            </div>
...
const PayoutsTable = ({ openSettlement, handleBulkPayout }) => (
    <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white/50 shadow-xl overflow-hidden min-h-[60vh] flex flex-col animate-slide-up">
        <div className="p-6 border-b border-white/50 flex justify-between items-center">
            <h3 className="text-lg font-black text-[#2D241E]">Kitchen Settlements</h3>
            <button onClick={handleBulkPayout} className="px-5 py-2 bg-[#2D241E] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-orange-600 transition-colors">Process Bulk Payout</button>
        </div>
        <div className="overflow-x-auto flex-1 custom-scrollbar">
            <table className="w-full text-left border-collapse">
                <thead className="bg-white/50 sticky top-0 z-10 backdrop-blur-md">
                    <tr>
                        <th className="px-8 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Kitchen</th>
                        <th className="px-6 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Amount</th>
                        <th className="px-6 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Orders</th>
                        <th className="px-6 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Status</th>
                        <th className="px-8 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest text-right">Action</th>
                    </tr>
                </thead>
                    ...
                    {payoutsData.map((p) => (
                        <tr key={p.id} className="group hover:bg-white/80 transition-all cursor-pointer" onClick={() => openSettlement(p)}>
                            <td className="px-8 py-5">
                                <p className="text-xs font-black text-[#2D241E]">{p.kitchen}</p>
                                <p className="text-[9px] font-bold text-[#897a70] mt-0.5">ID: {p.id}</p>
                            </td>
                            ...
                            <td className="px-6 py-5">
                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wide border ${p.status === 'Paid' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>
                                    {p.status}
                                </span>
                            </td>
...
const InvoicesTable = ({ openInvoice }) => {
    ...
    return (
        <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white/50 shadow-xl overflow-hidden min-h-[60vh] flex flex-col animate-slide-up">
            <div className="p-6 border-b border-white/50 flex justify-between items-center">
                <h3 className="text-lg font-black text-[#2D241E]">Recent Invoices</h3>
                <div className="flex gap-2">
                    <input
                        ...
                        className="bg-white/50 border border-white px-4 py-2 rounded-xl text-[10px] font-bold outline-none focus:ring-2 focus:ring-[#2D241E]/10 w-64"
                    />
                </div>
            </div>
            <div className="overflow-x-auto flex-1 custom-scrollbar">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-white/50 sticky top-0 z-10 backdrop-blur-md">
                        <tr>
                            <th className="px-8 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Invoice #</th>
                            <th className="px-6 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">User</th>
                            <th className="px-6 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Amount</th>
                            <th className="px-6 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest">Status</th>
                            <th className="px-8 py-5 text-[9px] font-black text-[#897a70] uppercase tracking-widest text-right">Preview</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100/50">
                        {filteredInvoices.length > 0 ? (
                            filteredInvoices.map((inv) => (
                                <tr key={inv.id} className="group hover:bg-white/80 transition-all cursor-pointer" onClick={() => openInvoice(inv)}>
                                    <td className="px-8 py-5 text-xs font-black text-[#2D241E] font-mono">{inv.id}</td>
                                    <td className="px-6 py-5">
                                        <p className="text-xs font-bold text-[#2D241E]">{inv.user}</p>
                                        <p className="text-[9px] text-[#897a70]">{inv.date}</p>
                                    </td>
                                    <td className="px-6 py-5 text-xs font-bold text-[#2D241E]">{inv.amount}</td>
                                    <td className="px-6 py-5">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wide border ${inv.status === 'Paid' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : inv.status === 'Overdue' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-gray-50 text-gray-600 border-gray-100'}`}>
                                            {inv.status}
                                        </span>
                                    </td>
                                    <td className="px-8 py-5 text-right">
...
const TaxComplianceView = () => (
    <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white/40 rounded-[2.5rem] border border-white/50 backdrop-blur-md animate-slide-up">
        ...
        <h3 className="text-xl font-black text-[#2D241E]">Tax Compliance Hub</h3>
...
const SettlementModal = ({ data, onClose }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div className="bg-[#F5F2EB] rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-[scaleIn_0.3s] relative z-10 border border-white/20 flex flex-col max-h-[90vh]">
                ...
                <div className="p-8 pb-4 bg-white/80 backdrop-blur-xl border-b border-[#2D241E]/5 flex justify-between items-start shrink-0 relative z-10">
                    <div>
                        <h3 className="text-2xl font-black text-[#2D241E]">Settlement Authorization</h3>
                        <p className="text-[10px] font-bold text-[#897a70] mt-1 uppercase tracking-widest">ID: {data.id} • {data.date}</p>
                    </div>
                ...
                <div className="p-8 pt-2 flex-1 relative z-10">
                    ...
                    {step === 0 && (
                        <div className="space-y-6 animate-enter">
                            <div className="p-6 bg-[#2D241E] rounded-[2rem] text-white flex justify-between items-center shadow-xl">
                                <div>
                                    <p className="text-[10px] font-black text-white/50 uppercase tracking-widest">Net Payable</p>
                                    <h2 className="text-4xl font-black tracking-tighter mt-1">{data.amount}</h2>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Bank Verified</p>
                            ...
                            <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                                <div className="flex justify-between mb-4">
                                    <span className="text-[10px] font-black text-[#5C4D42] uppercase tracking-widest">Platform Commission</span>
                                    <span className="text-xs font-black text-[#2D241E]">{commission}%</span>
                                </div>
                                <input
                                    type="range"
                                    min="0"
                                ...
                                />
                                <p className="text-[9px] font-bold text-[#897a70] mt-2 text-center">Adjusting commission affects the Net Payable amount.</p>
                            </div>

                            <button onClick={handlePayout} className="w-full py-4 bg-emerald-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-600 hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-emerald-500/30">
                                Authorize Transfer
                            </button>
                        </div>
                    )}
                    ...
                    {step === 1 && (
                        <div className="flex flex-col items-center justify-center h-64 animate-enter">
                            ...
                            <h4 className="text-lg font-black text-[#2D241E]">Processing Transfer...</h4>
                    ...
                    {step === 2 && (
                        <div className="flex flex-col items-center justify-center h-64 animate-enter">
                            ...
                            <h4 className="text-2xl font-black text-[#2D241E]">Transfer Complete!</h4>
...
const InvoicePreviewModal = ({ data, onClose }) => {
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-[#2D241E]/80 backdrop-blur-md animate-[fadeIn_0.3s]" onClick={onClose}></div>
            <div className="bg-[#F5F2EB] rounded-[2rem] w-full max-w-lg h-[80vh] overflow-hidden shadow-2xl animate-[scaleIn_0.3s] relative z-10 flex flex-col border border-white/20">
                ...
                <div className="relative z-10 flex-1 bg-gray-50/50 overflow-y-auto p-8 custom-scrollbar">
                    <div className="bg-white shadow-sm p-8 min-h-[500px] text-[#2D241E]">
                        <div className="flex justify-between items-start mb-8">
                            <h1 className="text-2xl font-black tracking-tighter">INVOICE</h1>
                            <div className="text-right">
                        ...
                        <div className="border-b border-gray-100 pb-6 mb-6">
                            <div className="grid grid-cols-2">
                                <div>
                                    <p className="text-[9px] uppercase font-bold text-gray-400">Bill To</p>
                                    <p className="text-sm font-bold mt-1">{data.user}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] uppercase font-bold text-gray-400">Invoice #</p>
                                    <p className="text-sm font-bold mt-1">{data.id}</p>
                                    <p className="text-[9px] uppercase font-bold text-gray-400 mt-2">Date</p>
                        ...
                        <table className="w-full text-left mb-8">
                            <thead>
                                <tr className="border-b-2 border-gray-100">
                                    <th className="pb-2 text-[9px] font-black uppercase text-gray-400">Item</th>
                                    <th className="pb-2 text-[9px] font-black uppercase text-gray-400 text-right">Amount</th>
                                </tr>
                            </thead>
                        ...
                        <div className="flex justify-end">
                            <div className="text-right">
                                ...
                                <h3 className="text-2xl font-black text-[#2D241E] mt-1">{data.amount}</h3>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative z-10 p-4 bg-white border-t border-gray-100 flex gap-3 shadow-[0_-5px_15px_rgba(0,0,0,0.02)]">
                    <button className="flex-1 py-3 bg-[#2D241E] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-colors">Download PDF</button>
                    <button className="flex-1 py-3 bg-white border border-gray-200 text-[#2D241E] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-50 transition-colors">Email Invoice</button>
                </div>
            </div>
        </div>,
        document.body
