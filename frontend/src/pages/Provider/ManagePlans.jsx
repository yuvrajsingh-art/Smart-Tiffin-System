...
const ManagePlans = () => {
    ...
    const fetchPlans = async () => {
        try {
            setLoading(true);
            const res = await ProviderApi.get('/provider-plans');
            if (res.data.success) {
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (editingPlan) {
                await ProviderApi.put(`/provider-plans/${editingPlan._id}`, planData);
                toast.success("Plan updated successfully");
            } else {
                if (plans.length >= 1) {
                    toast.error("You can only have 1 plan. Delete existing plan to create new one.");
                    return;
                }
                await ProviderApi.post('/provider-plans', planData);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await ProviderApi.delete(`/provider-plans/${id}`);
            ...
            <div className="flex-1 flex flex-col overflow-auto">
                ...
                <div className="p-8 max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 mb-6">
                        <p className="text-sm font-bold text-orange-800">⚠️ You can only have 1 active plan. Delete existing plan to create a new one.</p>
                    </div>

                    {plans.length === 0 ? (
                        <div className="text-center py-12">
                            <p className="text-gray-500 font-bold">No plans yet. Create your first plan!</p>
                        </div>
                    ) : (
